...
check_prerequisites() {
    ...
    if ! docker info >/dev/null 2>&1; then
        log_step "PREREQUISITES" "ERROR" "Docker is not running"
        exit 1
    fi
...
stop_node() {
    ...
    if docker exec "$CONTAINER_NAME" pgrep octez-node >/dev/null 2>&1; then
        log_step "NODE_STOP" "INFO" "Tezos node is running, stopping gracefully..."

        if docker exec "$CONTAINER_NAME" pkill -TERM octez-node; then
...
import_snapshot() {
    ...
    if docker exec -i "$CONTAINER_NAME" octez-node snapshot import --no-check /dev/stdin < "$snapshot_file" 2>&1 | \
...
verify_import() {
    ...
    if docker exec "$CONTAINER_NAME" octez-node run --network "$NETWORK" --data-dir /var/lib/tezos &
    then
        local node_pid=$!
        sleep 10
        ...
        # Stop the test node
        docker exec "$CONTAINER_NAME" pkill octez-node || true
...
main() {
    ...
    log_step "IMPORT_COMPLETE" "INFO" "Next step: docker compose up -d"
}
