...
check_account_exists() {
    ...
    if docker exec "$CONTAINER_NAME" octez-client show address "$ACCOUNT_ALIAS" >/dev/null 2>&1; then
        local address
        address=$(docker exec "$CONTAINER_NAME" octez-client show address "$ACCOUNT_ALIAS" --show-secret | grep "Hash:" | awk '{print $2}')
        ...
        log_step "ACCOUNT_CHECK" "INFO" "Create account with: docker exec $CONTAINER_NAME octez-client gen keys $ACCOUNT_ALIAS"
        exit 1
    fi
}
...
check_account_balance() {
    ...
    if balance_tz=$(docker exec "$CONTAINER_NAME" octez-client get balance for "$ACCOUNT_ALIAS" 2>/dev/null | grep -oE '[0-9]+\.?[0-9]*' | head -1); then
...
register_delegate() {
    ...
    if docker exec "$CONTAINER_NAME" octez-client register key "$ACCOUNT_ALIAS" as delegate 2>&1 | tee -a "$LOG_FILE"; then
        ...
        if check_existing_registration "$address"; then
            ...
            log_step "DELEGATE_REGISTRATION" "INFO" "Check status later with: docker exec $CONTAINER_NAME octez-client rpc get /chains/main/blocks/head/context/delegates/$address"
        fi
...
check_rights() {
    ...
    if current_level=$(docker exec "$CONTAINER_NAME" octez-client rpc get /chains/main/blocks/head/header | jq -r '.level' 2>/dev/null); then
    ...
    local baking_rights
    if baking_rights=$(docker exec "$CONTAINER_NAME" octez-client rpc get "/chains/main/blocks/head/helpers/baking_rights?delegate=$address&max_priority=5&cycle=$((cycle+1))" 2>/dev/null | jq '. | length' 2>/dev/null); then
    ...
    if endorsing_rights=$(docker exec "$CONTAINER_NAME" octez-client rpc get "/chains/main/blocks/head/helpers/endorsing_rights?delegate=$address&cycle=$((cycle+1))" 2>/dev/null | jq '. | length' 2>/dev/null); then
